...
- **Multiple Bible translations:** ESV, NIV, NKJV, KJV, NLT selector in settings (ignore for now)
...
- **Animated Weather module:** Animated weather module (more details coming soon..)
- **Pray Button:** User presses "Pray" which will lock the screen with a set timer (user's preference) and once the timer is done user is able to return back to the daily activities. Can be counted as a "daily streak" once completed.
